 ...
 */
public interface ActivityHoldService {
	/**#展示所有自身可以进行签到的活动*/
	...
	/**#当第一个人签到时,表示活动已经进行了;修改活动开始执行时间*/
	 ...
	 ActUser findUserHaveApplyAdvanceActivityByActidAndUserid(Integer actId2,Integer userId2);
	 /**#若没有参与过预参与,则插入新数据; 设置为已经参与过预参与*/
	 void saveSpotSignInfo(Integer actId2,Integer userId2);
	/** #如已经参与过预参与,则改变实际参与状态 */
	 void modifyUserHaveApplyAdvanceActivityActUserByActidAndUserid(Integer actId2,Integer userId2);
	 /**#判断用户是否已经参与过签到*/
