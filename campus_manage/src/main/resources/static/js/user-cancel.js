...
//			        	}
			        ...
			        {"data":'userId',
			        	render : function(data){
			        		return '<button type="button" data-user-id1="'+data+'" onclick="removeUserByUserId(this)" >删除</button>';
			        	}
			        },
			    ],
 ...
 * 删除用户
 */
function removeUserByUserId(obj){
	var AAA=confirm("确认删除用户?");
	if(!AAA){  return ;  };
	//获取id值
	var user=$(obj).attr("data-user-id1");
	var userLevel=$(obj).parents("tr").find("td:eq(3)").html();
	//
	$.ajax({
		url: "/user/removeUserByUserId",
